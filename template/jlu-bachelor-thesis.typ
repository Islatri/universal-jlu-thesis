#import "../lib.typ": jlu-bachelor, formula
...
#show: jlu-bachelor.with(
  ...
  // 版权使用授权书参数
  authorization-author-sig: "../assets/images/logo.png", // 作者签名图片路径
  authorization-mentor-sig: "../assets/images/logo.png", // 指导教师签名图片路径
  authorization-signature-height: 2.5em,

  // 承诺书参数
  commitment-signature-img: "../assets/images/logo.png", // 签名图片路径
...
本研究在多个方面实现了技术创新。首先，我们在*萌感知特征提取*方面取得了突破，提出了专门用于捕捉二次元角色萌属性的特征提取方法，相比传统方法提升了45%的识别精度@our_method2024。其次，我们设计了*Ciallo激活函数*，其公式为 #formula[$ f(x) = max(0, x) times "kawaii_factor" $][]，这个创新的激活函数能够保持萌特征的纯真性，避免过度平滑@ciallo_activation2024。第三，我们提出了*爱心损失函数*，这是一个融合了情感因子的损失函数，使模型能够更好地理解和学习角色的情感特征@love_loss2024。最后，我们构建了*多尺度萌特征金字塔*，这是一个专门用于捕捉不同尺度萌特征的特征金字塔网络，能够在多个分辨率上同时进行特征学习@moe_pyramid2024。
...
#formula[$ M(C) = sum_(i=1)^n w_i times A_i(C) times F_i(C) $][]
...
考虑到萌文化的动态发展特性，我们提出了动态萌属性演化理论@dynamic_moe2024 。在*时间演化模型*方面，我们使用公式 #formula[$ A_t = A_(t-1) + alpha times Delta A_t + beta times N_t $][] 来描述萌属性随时间的演化过程，其中 $A_t$ 表示时刻 $t$ 的萌属性状态，$Delta A_t$ 表示萌属性的变化趋势，$N_t$ 表示新涌现的萌属性，$alpha, beta$ 为演化参数。在*适应性学习机制*方面，我们通过持续学习技术，使AI系统能够识别新出现的萌属性类型，适应萌文化的发展变化，并保持对经典萌属性的准确理解。这套完整的理论体系为我们的MoeMoe-CNN提供了坚实的理论基础，确保AI系统能够真正理解和掌握萌文化的精髓～Ciallo～(∠・ω< )⌒☆
...
#figure(
  image("../assets/images/logo.png", width: 100%),
...
在*多尺度萌特征提取*方面，我们使用公式 #formula[$ F_"moe"^l = sigma(W_"moe" * F^(l-1) + b_"moe" + alpha_"moe" times "MoeContext"^l) $][] 进行特征计算，其中 $F_"moe"^l$ 为第 $l$ 层的萌特征，$W_"moe"$ 为萌感知权重矩阵，"MoeContext"^l 为萌上下文信息，$alpha_"moe"$ 为萌感知强度系数。

在*萌上下文感知机制*方面，我们通过分析角色周围的环境信息，使用公式 #formula[$ "MoeContext"^l = "AttentionMoe"(F^(l-1), "ContextFeature"^l) $][] 来增强萌特征的识别精度。这种多层次的设计方式能够更加全面地捕捉角色的萌属性信息。

=== 可爱度池化层

传统的最大池化和平均池化无法保留萌特征的关键信息，我们提出了可爱度池化@kawaii_pooling2024 。在*可爱度权重计算*方面，我们使用公式 #formula[$ w_(i,j) = "softmax"("KawaiiScore"(f_(i,j))) $][] 来计算每个特征位置的权重，其中 $"KawaiiScore"$ 函数评估每个特征的可爱程度。

在*加权池化操作*方面，我们使用公式 #formula[$ "KawaiiPool"(F) = sum_(i,j) w_(i,j) times f_(i,j) times "PurityFactor" $][] 进行池化，其中"PurityFactor"确保保留萌特征的纯真性，防止在池化过程中丧失重要的萌信息。
...
传统的随机初始化无法体现对萌文化的理解，我们提出了爱心权重初始化策略@love_init2024 。在*爱心分布权重*方面，我们基于心形函数生成初始权重分布，使用公式 #formula[$ w_"init" = "HeartFunction"(r, theta) times "random_scale" $][] 来初始化网络权重。其中心形函数定义为 #formula[$ "HeartFunction"(r, theta) = r times (1 - cos(theta)) times sin(theta) $][]，这个函数生成的权重分布形成心形的分布模式，象征着我们对萌文化的爱心。
...
标准的梯度下降算法缺乏对萌特征学习的针对性，我们设计了萌梯度下降算法@moe_sgd2024 。在*萌梯度计算*方面，我们使用公式 #formula[$ g_"moe" = g_"standard" + alpha times g_"moe_reg" + beta times g_"ciallo" $][] 来计算梯度，其中 $g_"standard"$ 是标准梯度，$g_"moe_reg"$ 是萌正则化梯度，$g_"ciallo"$ 是Ciallo精神梯度。这种组合方式使得模型在学习基本特征的同时，也能够学到萌属性的特殊知识。

在*自适应学习率*方面，我们使用公式 #formula[$ lr_t = lr_0 times (1 + "moe_progress" times cos("training_phase")) $][] 来动态调整学习率，学习率随训练阶段自适应调整，在萌特征学习的关键阶段提高学习效率。

在*爱心动量机制*方面，我们使用公式 #formula[$ v_t = gamma times v_(t-1) + (1-gamma) times g_"moe" times "love_factor" $][] 来计算动量，其中"love_factor"根据当前损失的萌相关程度动态调整。这个机制能够保证模型在学习过程中始终朝着正确的方向前进。

=== Ciallo正则化技术

为了防止模型过拟合并保持萌特征的纯真性，我们提出了Ciallo正则化@ciallo_reg2024 。在*萌纯真度正则化*方面，我们使用公式 #formula[$ L_"purity" = lambda_1 times sum_i |w_i - "PureWeight"_i|^2 $][] 来计算，这鼓励模型权重接近纯真的萌特征表示，防止模型学到虚假或扭曲的特征。

在*萌多样性正则化*方面，我们使用公式 #formula[$ L_"diversity" = lambda_2 times sum_(i,j) "Similarity"(f_i, f_j) times "DiversityPenalty" $][] 来促进模型学习多样化的萌属性表示，确保模型能够区分不同类型的萌特征而不是把所有特征都学成一样。

在*Ciallo一致性正则化*方面，我们使用公式 #formula[$ L_"consistency" = lambda_3 times ||f("AugmentedInput") - f("OriginalInput")||_2 $][] 来确保模型对同一角色的不同表现形式保持一致的萌理解。这种一致性约束能够提高模型的鲁棒性和泛化能力。
...
我们设计了复合损失函数来综合优化多个目标。*分类损失*使用标准的交叉熵损失 #formula[$ L_"classification" = -sum_i y_i log(hat(y)_i) $][] 来优化角色分类。*萌感知损失*通过 #formula[$ L_"moe" = sum_k "MoeWeight"_k times |"PredictedMoe"_k - "TrueMoe"_k| $][] 来直接优化萌属性预测准确性。*Ciallo精神损失*使用 #formula[$ L_"ciallo" = "PurityLoss" + "InclusivityLoss" + "CreativityLoss" $][] 来确保模型保持纯真、包容和创意。*正则化损失*采用 #formula[$ L_"reg" = L_"purity" + L_"diversity" + L_"consistency" $][] 来防止过拟合和保持多样性。

== 多模态萌度评估模块

=== 视觉萌度评估

针对视觉信息的萌度评估，我们设计了专门的评估网络@visual_moe2024 。在*萌特征提取*方面，使用改进的注意力机制提取关键萌特征，公式为 #formula[$ F_"visual" = "MoeAttention"("ConvFeatures", "MoeQuery") $][] 在*萌度评分*方面，通过 #formula[$ "MoeScore"_"visual" = "MLP"([F_"visual", "ContextInfo", "StyleInfo"]) $][] 综合多种信息进行评分。在*置信度评估*方面，使用 #formula[$ "Confidence" = "Sigmoid"("UncertaintyEstimation"(F_"visual")) $][] 来量化模型对评分的置信程度。

=== 文本萌度评估

对于角色对话和描述文本的萌度评估@text_moe2024 ，我们采用了多层次的方法。在*萌语言模型*方面，基于Transformer架构，专门用于理解萌表达，公式为 #formula[$ "MoeEmbedding" = "Transformer"("TokenSequence", "MoeContext") $][] 在*萌情感分析*方面，通过 #formula[$ "MoeEmotion" = "EmotionClassifier"("MoeEmbedding") $][] 进行细致的情感识别。在*萌语言特征*方面，我们提取萌相关的语言特征，如语调、用词习惯、表达方式等，这些特征能够深层次地反映文本的萌度。
...
#figure(
  image("../assets/images/logo.png", width: 90%),
